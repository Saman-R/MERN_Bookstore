...
const BookSingle = ({ book }) => {
    ...
    return (
        <div
            key={book._id}
            className='rounded-lg px-4 py-2 m-4 relative hover:shadow-xl '
        >
            <h2 className='absolute top-1 right-2 px-4 py-1 bg-sky-300 rounded-xl'>
                {book.publishYear}
            </h2>
            <h4 className='my-2 text-gray-500'> {book._id}</h4>
            <div className='flex justify-start item-center gap-x-2'>
                <PiBookOpenTextLight className='text-sky-300 text-xl' />
